# 1) Create a Food class with a “name” and a “kind” attribute 
# as well as a “describe()” method (which prints “name” and “kind” in a sentence).
from oop_assignment.food import Food
from oop_assignment.meat import Meat
from oop_assignment.fruit import Fruit 

my_food = Food('Bread','Staple')

my_food.describe()

# 2) Try turning describe() from an instance method into a class and a static method. 
# Change it back to an instance method thereafter.

# 3) Create a  “Meat” and a “Fruit” class – both should inherit from “Food”. 
# Add a “cook()” method to “Meat” and “clean()” to “Fruit”.
m = Meat('chicken', 'Non_Veg')
m.describe()
m.cook()

f = Fruit('apple', 'Veg')
f.describe()
f.clean()


# 4) Overwrite a “dunder” method to be able to print your “Food” class.
print('__ food __')
print(my_food)

print('__ meat __')
print(m)

print('__ fruit __')
print(f)